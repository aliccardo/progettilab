...
* jQuery3
* Zurb Foundation 6.5
* [LDAP Authentication](https://github.com/cschiewek/devise_ldap_authenticatable) *
* [Openssl](https://www.openssl.org/)

\* In alternativa ad LDAP è possibile utilizzare altri sistemi di autenticazione, come [CAS](http://rubycas.github.io/)
...
* Autenticazione utenti: LDAP (o Active Directory)
...
Successivamente usare il comando "rails credentials:edit" per modificare il file
  ...
  # Utilizzato per l'autenticazione e per la sincronizzazione dei dati degli utenti con il database del personale. Procedura path 'app/models/user.rb'
  ldap:
    domain: @dominio.di.autenticazione.ad'
    host: server.ldap
    port: 389
    admin: utente_per_ldap
    password: password_per_ldap
    base: OU=organizzazione2,OU=organizzazione1,DC=dominio2,DC=dominio1,DC=dominiotop
    attributes:
      - sn
      - givenName
      - mail
  ...
  mail:
    domain_name: 'nome_dominio.it'
    smtp: 'smtp.server.posta.it'
    port: porta_per_conessione
    sender: 'email_mittente'
    username: 'username'
    password: 'password'
    protocol: 'protocollo_link_nei_messaggi'
    url: 'url_base_link_nei_messaggi'
